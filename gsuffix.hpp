...
class Node {
    ...
    void get_substrings(std::set<std::pair<int, int>>& subs, int string_dept, int* most_frequent, int minimum);
...
template<typename T>
void Node<T>::print_tree(std::vector<T> current){
    for(auto [_, tree] : this->children){
        std::vector<T> c;
...
void Node<T>::get_substrings(std::set<std::pair<int, int>>& subs, int string_dept, int* most_frequent, int minimum){
    ...
    if(this->is_root() || string_dept < minimum || (word[this->end()] == SEPERATOR && string_dept == minimum))
        return;
    int len = this->is_leaf() ? string_dept - 1 : string_dept;    // inorder to remove the last seperator count.
